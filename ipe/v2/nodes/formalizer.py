...
- **graph canonical 구조 사실(불변·이것만 진실)**: weighted_edges/tree_edges 입력에는
  ① **self-loop(자기 자신으로의 간선)이 절대 없다** ② weighted_edges 는 다중 간선
  허용·연결 비보장(분리 컴포넌트 가능), tree_edges 는 트리(연결·무사이클) 보장.
  edge_case_semantics 는 **실제 발생 가능한** 구조에 대해서만 정의한다 — 다중 간선,
  분리(도달 불가), 시작==끝, V 하한. **self-loop 의 처리 의미를 정의하지 말 것**
  (입력에 없으므로). self-loop 동작을 invariant 에 적으면 narrative 가 그걸 서술하고
  형식 계약('self-loop 없음')·채점셋과 3중 모순을 일으켜 QA 가 reject 한다(N=18 실측).
