...
- **output_format 은 도메인 의미가 아니라 인쇄 구조만 서술**한다: 출력 타입·개수·형식
  (예: '한 줄에 정수 하나', '공백 구분 정수 여러 개'), 1-indexed/0-indexed 위치인지,
  해 없음 sentinel(-1 등)만 명시하라. **그 값이 도메인에서 무엇을 뜻하는지(예: '누적
  수확량' vs '단위 수확량', '최소 비용' vs '최대 이익')는 쓰지 말 것** — 답의 도메인
  의미는 narrative(지문)가 단일 저작한다. output_format 에 도메인 해석을 박으면
  narrative 가 같은 의미를 독립 서술해 'output_format↔description' 용어 드리프트로 QA
  ambiguity reject 된다(실측). 구조만 쓰면 어떤 도메인 서술과도 모순되지 않는다.
...
- **output_format 은 인쇄 구조만**(타입·개수·형식·sentinel) 서술하고, 그 값의 도메인
  의미는 쓰지 말 것 — 의미는 narrative 가 단일 저작한다(output_format↔description 용어
  드리프트 방지).
