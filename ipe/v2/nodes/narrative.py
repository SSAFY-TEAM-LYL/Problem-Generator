...
- output_invariants 가 정의한 **퇴화/경계 케이스 동작**(시작==끝, 도달 불가,
  ...
  단 **output_invariants 에 실제로 있는 케이스만** 서술한다 — 없는 동작을 지어내지 말
  것. 특히 그래프 입력에는 **self-loop(자기 자신으로의 간선)이 존재하지 않으므로**
  '자기 루프가 있을 수 있다/처리한다' 류를 **절대 쓰지 말 것** (형식 계약은 'self-loop
  없음'으로 고정 — 서술하면 3중 모순으로 QA reject). 발생 가능한 그래프 퇴화는 다중
  간선·분리(도달 불가)·시작==끝뿐이다.
